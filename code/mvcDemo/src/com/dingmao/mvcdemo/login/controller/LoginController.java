...
public class LoginController {
	...
	@ResponseBody
	public MessageVo checkuser(HttpServletRequest request, User loginUser,
			ModelMap modelMap) {
		MessageVo message = new MessageVo(true, "校验成功");
		User user = userService.checkUserPwd(loginUser);
		if (user == null)
			message = new MessageVo(false, "用户或密码有误，请重新输入");
		return message;
	}
