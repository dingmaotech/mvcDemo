...
public class HibernateDao<T, PK extends Serializable> implements
	...
	public int deleteById(PK id) {
		...
		Query query = session.createQuery(" delete  from "
				+ entityClass.getSimpleName() + "  where " + this.getIdName()
				+ "=?");
	...
	protected Integer countCriteriaResult(final Criteria c) {
		...
		try {
			...
			logger.error(
					"[jcommon] HibernateDao countCriteriaResult() error:{}",
					e.getMessage());
		}
		c.setCacheable(true);
		// 执行Count查询
	...
	public Page<T> findPage(Page<T> page, List<Parameter> parameters) {
		...
		// HibernateCriteria.getDetachedCriteria(parameters,entityClass);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@Override
	public Page<T> findPage(Page<T> page, String hql, Map<String, Object> map) {
		Assert.notNull(page, "page is not null!!!");
		Query q = createQuery(hql, map);
		if (page.isAutoCount()) {
			Integer totalCount = countHqlResult(hql, map);
			page.setTotalRows(totalCount);
		}
		setPageParameter(q, page);
		List result = q.list();
