package com.dingmao.platform.poi.excel.annotation;
...
public @interface Excel {
	/**
	 * 导出时的列名字 不能有annocation重名的 导出时列名的顺序由annotation的字段的顺序注解字段orderNum有关
	 * 可以使用a_id,b_id来确实是否使用
	 */
	public String exportName();

	/**
	 * 导出时在excel中每个列的宽度， 单位为字符，一个汉字=2个字符 当内容的宽度小于表头的列宽以表头宽度为准进行计算 如 性别 为4个字符
	 * ，男/女占2个字符，宽度至少设置为4 限制1-255
	 */
	...
	/**
	 * 若是sign为1,则需要在pojo中加入一个方法 convertGet字段名() 例如，字段sex ，需要加入 public String
	 * convertGet字段名() 返回值为string 若是sign为0,则不转换 建议使用 @Transient
	 * 这个注解让hibernate忽略这个方法
	 */
	public int exportConvertSign() default 0;

	/**
	 * 导入数据是否需要转化 及 对已有的excel，是否需要将字段转为对应的数据 若是sign为1,则需要在pojo中加入 void
	 * convertSet字段名(String text)
	 */
	public int importConvertSign() default 0;

	/**
	 * 导入导出是否都需要转化,同时设置exportConvertSign和importConvertSign
	 */
	 ...
	 * cell 写入的设置的函数
	 */
	public String cellFunction() default "";
