...
public final class ExcelExportOfTemplateUtil {
	 ...
	 * @return
	 */
	public static Workbook exportExcel(TemplateExportParam params,
			Class<?> clazz, Collection<?> data, Map<String, Object> map) {
		return createSheetInUserModel2FileByTemplate(params, clazz, data, map);
	}
	...
	private static Workbook createSheetInUserModel2FileByTemplate(
			TemplateExportParam params, Class<?> clazz, Collection<?> data,
			Map<String, Object> map) {
		// step 1. 判断模板的地址
		...
		try {
			...
			if (data != null) {
				// step 5. 正常的数据填充
				addDataToSheet(params, clazz, data, wb.getSheetAt(0), wb);
			}
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	 ...
	 * @date 2013-11-11
	 */
	private static Workbook getCloneWorkBook(TemplateExportParam params)
			throws Exception {
		Workbook wb = null;
	 ...
	 * @param workbook
	 */
	private static void addDataToSheet(TemplateExportParam params,
			Class<?> clazz, Collection<?> data, Sheet sheet, Workbook workbook)
			throws Exception {
	...
	private static int createCells(Drawing patriarch, int index, Object t,
			List<ExcelExportVo> excelParams, Sheet sheet, Workbook workbook)
			throws Exception {
	...
	private static void createStringCell(Row row, int index, String text,
			ExcelExportVo entity, Workbook workbook) {
	...
	private static void createImageCell(Drawing patriarch,
			ExcelExportVo entity, Row row, int i, String field, Object obj,
		...
		row.createCell(i);
		ClientAnchor anchor = workbook instanceof HSSFWorkbook ? new HSSFClientAnchor(
				0, 0, 0, 0, (short) i, row.getRowNum(), (short) (i + 1),
				row.getRowNum() + 1) : new XSSFClientAnchor(0, 0, 0, 0,
				(short) i, row.getRowNum(), (short) (i + 1),
				row.getRowNum() + 1);
	...
	private static void getExcelField(String targetId, Field field,
			ExcelExportVo excelEntity, Excel excel, Class<?> clazz)
			throws Exception {
		excelEntity.setName(getExcelName(excel.exportName(), targetId));
		...
		excelEntity.setType(excel.exportType());
		excelEntity.setCellFormula(excel.cellFunction());
		String fieldname = field.getName();
		excelEntity.setGetMethod(ExcelPublicUtil.getMethod(fieldname, clazz));
