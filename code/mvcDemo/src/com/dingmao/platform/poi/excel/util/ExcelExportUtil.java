...
import com.dingmao.platform.poi.excel.vo.TemplateExportParam;
import com.dingmao.platform.util.ApplicationContextUtil;
...
public final class ExcelExportUtil {
	 ...
	 *            输出流
	 */
	public static HSSFWorkbook exportExcel(ExcelTitleVo entity, Class<?> clazz,
			Collection<?> data) {
		HSSFWorkbook workbook = new HSSFWorkbook();
		createSheetInUserModel2File(workbook, entity, clazz, data);
		return workbook;
	}
	 ...
	 * @return
	 */
	public static Workbook exportExcel(TemplateExportParam param,
			Class<?> clazz, Collection<?> data, Map<String, Object> map) {
		return ExcelExportOfTemplateUtil.exportExcel(param, clazz, data, map);
	}
	...
	public static void createSheetInUserModel2File(HSSFWorkbook workbook,
		...
		try {
			...
			getAllExcelField(targetId, fileds, excelParams, clazz, null);
			...
			if (data != null && data.size() > 0) {
				...
				while (its.hasNext()) {
					Object t = its.next();
					index += createCells(patriarch, index, t, excelParams,
	 ...
	 * 对字段根据用户设置排序
	 */
	private static void sortAllParams(List<ExcelExportVo> excelParams) {
		Collections.sort(excelParams, new SortExcelFieldUtil());
		for (ExcelExportVo entity : excelParams) {
			if (entity.getList() != null) {
				Collections.sort(entity.getList(), new SortExcelFieldUtil());
			}
		}
	}
	...
	private static int createCells(Drawing patriarch, int index, Object t,
			List<ExcelExportVo> excelParams, Sheet sheet,
			HSSFWorkbook workbook, Map<String, HSSFCellStyle> styles)
			throws Exception {
		ExcelExportVo entity;
	 ...
	 * @throws Exception
	 */
	private static Object getCellValue(ExcelExportVo entity, Object obj)
			throws Exception {
		...
		if (!"".equals(value) && entity.getDictMap() != null
				&& entity.getDictMap().size() > 0) {
	...
	private static void createListCells(Drawing patriarch, int index,
			int cellNum, Object obj, List<ExcelExportVo> excelParams,
	...
	private static void setCellWith(List<ExcelExportVo> excelParams, Sheet sheet) {
		int index = 0;
		for (int i = 0; i < excelParams.size(); i++) {
			if (excelParams.get(i).getList() != null) {
				List<ExcelExportVo> list = excelParams.get(i).getList();
	 ...
	 * @param index
	 */
	private static void createTitleRow(ExcelTitleVo title, Sheet sheet,
			HSSFWorkbook workbook, int index, List<ExcelExportVo> excelParams) {
		Row row = sheet.createRow(index);
		...
		for (int i = 0, exportFieldTitleSize = excelParams.size(); i < exportFieldTitleSize; i++) {
			ExcelExportVo entity = excelParams.get(i);
			createStringCell(row, cellIndex, entity.getName(), titleStyle,
					entity);
			if (entity.getList() != null) {
				List<ExcelExportVo> sTitel = entity.getList();
				sheet.addMergedRegion(new CellRangeAddress(index, index,
	...
	private static void createStringCell(Row row, int index, String text,
			CellStyle style, ExcelExportVo entity) {
	...
	private static void createImageCell(Drawing patriarch,
			ExcelExportVo entity, Row row, int i, String string, Object obj)
			throws Exception {
		...
		if (entity.getExportImageType() == 1) {
			...
			try {
				String path = PropertiesUtil.getInstance().getPropertiesValue(
						"file.upload.path")
						+ string;
	 ...
	 * @param feildWidth
	 */
	private static int createHeaderRow(ExcelTitleVo entity, Sheet sheet,
	 ...
	 * @return
	 */
	private static int getFieldWidth(List<ExcelExportVo> excelParams) {
	...
	private static void getAllExcelField(String targetId, Field[] fields,
			List<ExcelExportVo> excelParams, Class<?> clazz,
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (ExcelPublicUtil.isCollection(field.getType())) {
				...
				List<ExcelExportVo> list = new ArrayList<ExcelExportVo>();
				getExcelFieldList(targetId,
						ExcelPublicUtil.getClassFields(clz), clz, list, null);
				entity = new ExcelExportVo();
				entity.setName(getExcelName(excel.exportName(), targetId));
				entity.setOrderNum(getCellOrder(excel.orderNum(), targetId));
				entity.setGetMethod(ExcelPublicUtil.getMethod(field.getName(),
						clazz));
				entity.setList(list);
				...
				entity = new ExcelExportVo();
				entity.setType(excel.exportType());
				getExcelField(targetId, field, entity, excel, clazz);
				if (getMethods != null) {
					List<Method> newMethods = new ArrayList<Method>();
					newMethods.addAll(getMethods);
					newMethods.add(entity.getGetMethod());
					entity.setGetMethods(newMethods);
				}
				excelParams.add(entity);
			} else {
				List<Method> newMethods = new ArrayList<Method>();
				...
				newMethods
						.add(ExcelPublicUtil.getMethod(field.getName(), clazz));
	...
	private static void getExcelFieldList(String targetId, Field[] fields,
			Class<?> clazz, List<ExcelExportVo> list, List<Method> getMethods)
			throws Exception {
		ExcelExportVo entity;
		for (int i = 0; i < fields.length; i++) {
			Field field = fields[i];
	...
	public static void getExcelField(String targetId, Field field,
			ExcelExportVo entity, Excel excel, Class<?> clazz) throws Exception {
		entity.setName(getExcelName(excel.exportName(), targetId));
		entity.setWidth(excel.exportFieldWidth());
		entity.setHeight(excel.exportFieldHeight());
		entity.setNeedMerge(excel.needMerge());
		entity.setOrderNum(getCellOrder(excel.orderNum(), targetId));
		entity.setWrap(excel.isWrap());
		entity.setExportImageType(excel.imageType());
		entity.setExportFormat(StringUtils.isNotEmpty(excel.exportFormat()) ? excel
				.exportFormat() : excel.imExFormat());
		String fieldname = field.getName();
		entity.setGetMethod(ExcelPublicUtil.getMethod(fieldname, clazz));
		if (excel.exportConvertSign() == 1 || excel.imExConvert() == 1) {
			...
			entity.setGetMethod(getConvertMethod);

			if (StringUtils.isNotEmpty(excel.dicCode())) {
				DictService dictService = ApplicationContextUtil.getContext()
						.getBean(DictService.class);
				Map<String, Map<String, String>> dictMap = new HashMap<String, Map<String, String>>();
				dictMap = dictService.getAllDicMap();
				entity.setDictMap(dictMap.get(excel.dicCode()));
			}
		}
	}
	...
	private static CellStyle getStyles(Map<String, HSSFCellStyle> map,
			boolean needOne, boolean isWrap) {
		if (needOne && isWrap) {
