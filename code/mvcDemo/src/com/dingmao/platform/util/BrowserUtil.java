...
import com.dingmao.platform.enums.BrowserEnum;
...
public class BrowserUtil {
	...
	public static BrowserEnum getBrowserType(HttpServletRequest request) {
		BrowserEnum browserType = null;
		if (getBrowserType(request, "rv:11.0"))
			browserType = BrowserEnum.IE11;
		if (getBrowserType(request, "MSIE 10.0"))
			browserType = BrowserEnum.IE10;
		if (getBrowserType(request, "MSIE 9.0"))
			browserType = BrowserEnum.IE9;
		if (getBrowserType(request, "MSIE 8.0"))
			browserType = BrowserEnum.IE8;
		if (getBrowserType(request, "MSIE 7.0"))
			browserType = BrowserEnum.IE7;
		if (getBrowserType(request, "MSIE 6.0"))
			browserType = BrowserEnum.IE6;
		if (getBrowserType(request, "Firefox"))
			browserType = BrowserEnum.Firefox;
		if (getBrowserType(request, "Safari"))
			browserType = BrowserEnum.Safari;
		if (getBrowserType(request, "Chrome"))
			browserType = BrowserEnum.Chrome;
		if (getBrowserType(request, "Opera"))
			browserType = BrowserEnum.Opera;
		if (getBrowserType(request, "Camino"))
			browserType = BrowserEnum.Camino;
		return browserType;
	}
