...
public class DetachedCriteriaUtil {
	...
	public static DetachedCriteria createDetachedCriteria(Class<?> pojoClazz,
		...
		return createDetachedCriteria(pojoClazz, startChar, alias, columnNames,
				null);
	}
	...
	private static Set<String> getAliasesFromRequest() {
		...
		Set<String> aliases = (Set<String>) request
				.getAttribute(ALIAS_KEY_IN_REQUEST);
		if (aliases == null) {
			aliases = new HashSet<String>(5);
			setAliasToRequest(request, aliases);
		}
		return aliases;
	}
	...
	private static boolean getHasJoinTatleFromRequest() {
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder
				.getRequestAttributes()).getRequest();
		Boolean hasJoin = (Boolean) request
				.getAttribute(HAS_JOIN_TABLE_KEY_IN_REQUEST);
		return hasJoin == null ? false : hasJoin;
	}

	/**
	 * 该方法提供DetachedCriteria对查询字段的封装， 2008-9-29 2009.9.9修改后，可支持无限级联取部分字段，如取如下字段
	 ...
	 */
	/*
	 * public static void selectColumn(DetachedCriteria criteria, String[]
	 * columnNames, Class<?> pojoClass, boolean forJoinTable) { if (null ==
	 * columnNames) { return; }
	 * 
	 * // 使用这个临时变量集合，是因为dinstinct关键字要放在最前面，而distinct关键字要在下面才决定放不放，
	 * List<Projection> tempProjectionList = new ArrayList<Projection>();
	 * 
	 * Set<String> aliases = getAliasesFromRequest(); boolean hasJoniTable =
	 * false; String rootAlias = criteria.getAlias(); for (String property :
	 * columnNames) { if (property.contains("_")) { String[] propertyChain =
	 * property.split("_"); createAlias(criteria, rootAlias, aliases,
	 * propertyChain, 0); tempProjectionList.add(Projections.property(
	 * StringUtil.getProperty(property)).as( StringUtil.getProperty(property)));
	 * hasJoniTable = true; } else {
	 * tempProjectionList.add(Projections.property( rootAlias + POINT +
	 * property).as(property)); } }
	 * 
	 * projectionList = Projections.projectionList(); if (hasJoniTable ||
	 * forJoinTable || getHasJoinTatleFromRequest()) {//
	 * 这个一定要放在tempProjectionList的前面，因为distinct要在最前面
	 * projectionList.add(Projections.distinct(Projections.id())); }
	 * 
	 * for (Projection proj : tempProjectionList) { projectionList.add(proj); }
	 * 
	 * criteria.setProjection(projectionList);
	 * 
	 * if (!hasJoniTable) {
	 * criteria.setResultTransformer(Transformers.aliasToBean(pojoClass)); }
	 * else {// 下面这个是自定义的结果转换器 criteria.setResultTransformer(new
	 * AliasToBean(pojoClass)); } }
	 */
