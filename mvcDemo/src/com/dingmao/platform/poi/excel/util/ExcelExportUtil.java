...
public final class ExcelExportUtil {
	 ...
	 *            输出流
	 */
	public static HSSFWorkbook exportExcel(ExcelTitleVo entity,
	...
	public static Workbook exportExcel(TemplateExportParam param,
		...
		return ExcelExportOfTemplateUtil.exportExcel(param, clazz,
				data, map);
	}
	...
	public static Workbook exportExcel(TemplateExportParam param,
		...
		return ExcelExportOfTemplateUtil.exportExcel(param, null,
				null, map);
	}
	 ...
	 * @throws Exception
	 */
	private static Object getCellValue(ExcelExportVo entity, Object obj) throws Exception{
		...
		if(!"".equals(value) && entity.getDictMap() != null && entity.getDictMap().size() > 0) {
	...
	private static void setCellWith(List<ExcelExportVo> excelParams,
			Sheet sheet) {
		int index = 0;
	 ...
	 * @param index
	 */
	private static void createTitleRow(ExcelTitleVo title, Sheet sheet, HSSFWorkbook workbook,
			int index, List<ExcelExportVo> excelParams) {
		Row row = sheet.createRow(index);
	 ...
	 * @throws Exception 
	 */
	private static void createImageCell(Drawing patriarch, ExcelExportVo entity, Row row,
		...
		if(entity.getExportImageType()==1){
			...
			try {
				String path = PropertiesUtil.getInstance().getPropertiesValue("file.upload.path") +string;
	...
	private static void getAllExcelField(String targetId, Field[] fields,
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (ExcelPublicUtil.isCollection(field.getType())) {
				...
				entity.setName(getExcelName(excel.exportName(),
						targetId));
				...
				getExcelField(targetId, field, entity, excel,
						clazz);
				if (getMethods != null) {
				...
				newMethods.add(ExcelPublicUtil.getMethod(field.getName(), clazz));
	...
	private static void getExcelFieldList(String targetId, Field[] fields,
			Class<?> clazz, List<ExcelExportVo> list,
			List<Method> getMethods) throws Exception {
		ExcelExportVo entity;
		for (int i = 0; i < fields.length; i++) {
			Field field = fields[i];
	...
	public static void getExcelField(String targetId, Field field,
			ExcelExportVo entity, Excel excel, Class<?> clazz)
			throws Exception {
		...
		entity.setExportFormat(StringUtils.isNotEmpty(excel.exportFormat())?
		...
		if (excel.exportConvertSign() == 1||excel.imExConvert()==1) {
			...
			if(StringUtils.isNotEmpty(excel.dicCode())){
				DictService dictService = ApplicationContextUtil.getContext().getBean(DictService.class);
	 ...
	 * @return
	 */
	public static HSSFCellStyle getTitleStyle(HSSFWorkbook workbook, ExcelTitleVo entity) {
	 ...
	 * @return
	 */
	public static HSSFCellStyle getHeaderStyle(HSSFWorkbook workbook, ExcelTitleVo entity) {
		HSSFCellStyle titleStyle = workbook.createCellStyle();
